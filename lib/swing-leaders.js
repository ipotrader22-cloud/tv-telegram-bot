...
function renderInternCards(items) {
  if (!items.length) return '<div class="empty">No potential candidates in this snapshot.</div>';
  return items.map(item => `
    <article class="intern-card">
      <div class="intern-top"><strong>${escapeHtml(item.ticker)}</strong><span class="score">${escapeHtml(item.score)} / 100</span></div>
...
<body>
  ...
  <main class="wrap">
    ...
    <section class="summary" aria-label="Swing Leaders summary">
      ...
      <div class="summary-card"><small>Interns</small><strong>${escapeHtml(data.intern_count)}</strong></div>
    ...
    <section class="how" aria-label="How Swing Leaders works">
      ...
      <div class="how-grid">
        ...
        <div class="how-item"><strong>Interns</strong><p>Potential candidates under active research review. They are not confirmed entries and may never enter the portfolio.</p></div>
    ...
    <section class="section">
      <div class="section-head"><div><h2>Interns</h2><p>Potential candidates under active research review.</p></div></div>
      <div class="intern-grid">${renderInternCards(data.interns)}</div>
    </section>
