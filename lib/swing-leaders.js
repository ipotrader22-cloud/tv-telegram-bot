...
function validatePublicFeed(snapshot) {
  ...
  requireText(snapshot.cash_pct, 'cash_pct');
  ...
  if (!Array.isArray(snapshot.active_positions)) throw new Error('Missing active_positions');
  if (!Array.isArray(snapshot.watchlist)) throw new Error('Missing watchlist');
  if (!Array.isArray(snapshot.closed_trades)) throw new Error('Missing closed_trades');
  if (snapshot.active_count !== snapshot.active_positions.length) throw new Error('active_count does not match Active Positions');
  if (snapshot.watch_count !== snapshot.watchlist.length) throw new Error('watch_count does not match Watchlist');

  const activeTickers = new Set();
  for (const item of snapshot.active_positions) {
    ...
    item.status = requireText(item.status, `${item.ticker} status`);
    if (item.status.toUpperCase() !== 'READY NOW') throw new Error(`Invalid active status for ${item.ticker}`);
    ...
    item.return_pct = requireText(item.return_pct, `${item.ticker} return_pct`);
  ...
  for (const item of snapshot.watchlist) {
    item.ticker = requireText(item.ticker, 'watch ticker').toUpperCase();
    item.score = scoreValue(item.score, `${item.ticker} score`);
    item.classification = requireText(item.classification, `${item.ticker} classification`).toUpperCase();
    if (!['CLOSE TO BREAKOUT', 'EARLY WATCH'].includes(item.classification)) {
      throw new Error(`Invalid watch classification for ${item.ticker}`);
    }
    ...
    if (watchTickers.has(item.ticker)) throw new Error(`Duplicate watch ticker ${item.ticker}`);
    watchTickers.add(item.ticker);
  }
  ...
  for (const item of snapshot.closed_trades) {
    ...
    item.exit_reason = requireText(item.exit_reason, `${item.ticker} exit_reason`);
    if (activeTickers.has(item.ticker)) throw new Error(`Closed ticker ${item.ticker} is still active`);
    if (closedTickers.has(item.ticker)) throw new Error(`Duplicate closed ticker ${item.ticker}`);
    closedTickers.add(item.ticker);
  }
...
function parsePublicFeed(values) {
  ...
  const activeIndex = sectionIndex(rows, 'ACTIVE POSITIONS');
  const watchIndex = sectionIndex(rows, 'WATCHLIST');
  const closedIndex = sectionIndex(rows, 'CLOSED TRADES');
  if (activeIndex < 0 || watchIndex < 0 || closedIndex < 0) throw new Error('Public Feed sections are incomplete');
  if (!(activeIndex < watchIndex && watchIndex < closedIndex)) throw new Error('Public Feed sections are out of order');
  ...
  const snapshot = {
    ...
    watch_count: integerValue(metadata.watch_count, 'watch_count'),
    ...
    active_positions: parseSection(rows, activeIndex, watchIndex, ACTIVE_FIELDS),
    watchlist: parseSection(rows, watchIndex, closedIndex, WATCH_FIELDS),
    closed_trades: parseSection(rows, closedIndex, rows.length, CLOSED_FIELDS),
  };

  return validatePublicFeed(snapshot);
}
...
function returnClass(value) {
  const number = Number(cleanText(value).replace('%', '').replace('+', ''));
...
function renderActiveRows(items) {
  return items.map(item => `
    <tr>
      ...
      <td data-label="Status"><span class="status ready">${escapeHtml(item.status.toUpperCase())}</span></td>
...
function renderWatchCards(items) {
  if (!items.length) return '<div class="empty">No watch candidates in this snapshot.</div>';
  return items.map(item => `
    <article class="watch-card">
      <div class="watch-top"><strong>${escapeHtml(item.ticker)}</strong><span class="score">${escapeHtml(item.score)} / 100</span></div>
      <span class="status watch">${escapeHtml(item.classification)}</span>
      <p>${escapeHtml(item.brief_reason)}</p>
      <small>Reviewed ${escapeHtml(item.review_date)}</small>
    </article>`).join('');
}
...
function renderSwingLeadersHtml(snapshot, { stale = false } = {}) {
  const data = validatePublicFeed(cloneSnapshot(snapshot));
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <meta name="description" content="Vixale Swing Leaders research/model portfolio: current picks, breakout watchlist, market posture, and closed trades." />
  <style>
    :root{--bg:#f7faf8;--card:#fff;--ink:#101713;--muted:#68756e;--line:#dfe8e3;--green:#087a48;--green-soft:#eaf7f0;--red:#a33b45;--red-soft:#fff0f1;--blue:#315c9b;--blue-soft:#eef4ff;--shadow:0 18px 52px rgba(16,23,19,.07)}
    *{box-sizing:border-box}body{margin:0;background:linear-gradient(180deg,#fbfdfc 0,var(--bg) 42%,#fff 100%);color:var(--ink);font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}a{color:inherit}.wrap{width:min(1180px,calc(100% - 36px));margin:auto}.topbar{border-bottom:1px solid var(--line);background:rgba(255,255,255,.88)}.topbar .wrap{min-height:66px;display:flex;align-items:center;justify-content:space-between;gap:18px}.brand{font-weight:650;letter-spacing:-.02em;text-decoration:none}.nav{display:flex;gap:16px;align-items:center}.nav a{color:var(--muted);font-size:13px;text-decoration:none}.hero{padding:62px 0 34px}.eyebrow{font-size:12px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:var(--green)}h1{font-size:clamp(42px,7vw,72px);line-height:.98;letter-spacing:-.055em;margin:14px 0 20px;font-weight:600}.hero-copy{max-width:820px;color:var(--muted);font-size:18px;line-height:1.6}.stamp{display:flex;flex-wrap:wrap;gap:9px;margin-top:22px}.pill,.status{display:inline-flex;align-items:center;border:1px solid var(--line);border-radius:999px;padding:7px 10px;font-size:11px;font-weight:700;letter-spacing:.045em;text-transform:uppercase;background:#fff}.pill.stale{border-color:#e2c88c;background:#fff8e8;color:#7b5b14}.summary{display:grid;grid-template-columns:1.35fr repeat(3,.55fr);gap:12px;margin:6px 0 38px}.summary-card{padding:20px;border:1px solid var(--line);border-radius:22px;background:rgba(255,255,255,.94);box-shadow:var(--shadow)}.summary-card small{display:block;color:var(--muted);font-size:11px;letter-spacing:.055em;text-transform:uppercase;margin-bottom:9px}.summary-card strong{font-size:24px;font-weight:600;letter-spacing:-.03em}.summary-card.posture strong{font-size:16px;line-height:1.45;letter-spacing:-.01em}.section{margin:0 0 24px;border:1px solid var(--line);border-radius:26px;background:rgba(255,255,255,.95);box-shadow:var(--shadow);overflow:hidden}.section-head{padding:22px 24px;border-bottom:1px solid var(--line);display:flex;align-items:end;justify-content:space-between;gap:18px}.section-head h2{margin:0;font-size:24px;letter-spacing:-.035em}.section-head p{margin:5px 0 0;color:var(--muted);font-size:13px}.table-wrap{overflow-x:auto}table{width:100%;border-collapse:collapse}th,td{padding:15px 14px;border-bottom:1px solid #edf2ef;text-align:left;vertical-align:top;font-size:13px}th{color:var(--muted);font-size:10px;letter-spacing:.065em;text-transform:uppercase;font-weight:700}tbody tr:last-child td{border-bottom:0}td span{display:block;color:var(--muted);font-size:11px;margin-top:4px}td.note{min-width:220px;color:#45514b;line-height:1.45}.gain{color:var(--green);font-weight:650}.loss{color:var(--red);font-weight:650}.flat{color:var(--muted)}.status.ready{border-color:#bce0cd;background:var(--green-soft);color:var(--green)}.status.watch{border-color:#cedcf2;background:var(--blue-soft);color:var(--blue);white-space:nowrap}.watch-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;padding:18px}.watch-card{border:1px solid var(--line);border-radius:20px;padding:18px;background:#fcfefd}.watch-top{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:13px}.watch-top strong{font-size:23px;letter-spacing:-.04em}.score{color:var(--muted);font-size:12px}.watch-card p{color:#46534c;font-size:13px;line-height:1.55;margin:14px 0}.watch-card small{color:var(--muted)}.disclosure{margin:34px 0 54px;padding:22px;border:1px solid var(--line);border-radius:20px;background:#fafcfb;color:var(--muted);font-size:12px;line-height:1.65}.disclosure strong{color:var(--ink)}.empty{padding:24px;color:var(--muted);text-align:center}.footer{padding:24px 0 42px;border-top:1px solid var(--line);color:var(--muted);font-size:12px}
    @media(max-width:900px){.summary{grid-template-columns:1fr 1fr}.summary-card.posture{grid-column:1/-1}.watch-grid{grid-template-columns:1fr}}
    @media(max-width:720px){.wrap{width:min(100% - 24px,1180px)}.hero{padding-top:42px}.nav a:first-child{display:none}.summary{grid-template-columns:1fr}.summary-card.posture{grid-column:auto}.section{border-radius:20px}.section-head{align-items:flex-start;flex-direction:column}table,thead,tbody,tr,th,td{display:block;width:100%}thead{display:none}tbody{padding:8px 14px}tr{padding:9px 0;border-bottom:1px solid var(--line)}tbody tr:last-child{border-bottom:0}td{display:grid;grid-template-columns:105px 1fr;gap:12px;padding:7px 2px;border:0;font-size:13px}td::before{content:attr(data-label);color:var(--muted);font-size:10px;letter-spacing:.055em;text-transform:uppercase}td.note{min-width:0}.status{justify-self:start}.watch-grid{padding:14px}}
  </style>
</head>
<body>
  ...
  <main class="wrap">
    <section class="hero">
      ...
      <p class="hero-copy">A read-only view of the Trading Lab Morning Leader model portfolio. Research classifications, scores, entries, exits, and market posture are displayed exactly from the approved research feed.</p>
      <div class="stamp"><span class="pill">Snapshot ${escapeHtml(data.snapshot_date)} · ${escapeHtml(data.snapshot_time_et)}</span><span class="pill">Quote source ${escapeHtml(data.quote_source)}</span>${stale ? '<span class="pill stale">Stale — last valid snapshot</span>' : ''}</div>
    </section>
    ...
    <section class="summary" aria-label="Swing Leaders summary">
      ...
      <div class="summary-card"><small>Active</small><strong>${escapeHtml(data.active_count)}</strong></div>
      <div class="summary-card"><small>Watching</small><strong>${escapeHtml(data.watch_count)}</strong></div>
      <div class="summary-card"><small>Cash</small><strong>${escapeHtml(data.cash_pct)}</strong></div>
    </section>

    <section class="section">
      <div class="section-head"><div><h2>Current Picks</h2><p>Active model positions from the approved Public Feed.</p></div><span class="status ready">READY NOW</span></div>
      <div class="table-wrap"><table><thead><tr><th>Ticker</th><th>Score</th><th>Entry</th><th>Current</th><th>Return</th><th>Status</th><th>Research Note</th></tr></thead><tbody>${renderActiveRows(data.active_positions)}</tbody></table></div>
    </section>

    <section class="section">
      <div class="section-head"><div><h2>Close to Breakout / Early Watch</h2><p>Watch candidates are supplied by Trading Lab; the website does not infer membership.</p></div></div>
      <div class="watch-grid">${renderWatchCards(data.watchlist)}</div>
    </section>

    <section class="section">
      <div class="section-head"><div><h2>Closed Trades</h2><p>Newest exit first.</p></div><span class="status">CLOSED</span></div>
    ...
    <aside class="disclosure"><strong>Research/model portfolio.</strong> ${escapeHtml(data.quote_delay_notice)} ${escapeHtml(data.research_disclaimer)} This page is not broker execution data and does not guarantee future performance.</aside>
  </main>
  <footer class="footer"><div class="wrap">Vixale Swing Leaders · Last Updated ${escapeHtml(data.snapshot_date)} ${escapeHtml(data.snapshot_time_et)} · Feed timestamp, not website fetch time.</div></footer>
</body>
</html>`;
}
