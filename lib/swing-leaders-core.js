...
function currencyValue(value, fieldName) {
  ...
  const standardMatch = text.match(new RegExp(`^([+-]?)\\$?(${numberPattern})$`));
  const accountingMatch = text.match(new RegExp(`^\\(\\$?(${numberPattern})\\)$`));
  if (!standardMatch && !accountingMatch) throw new Error(`Invalid currency for ${fieldName}`);

  const numericText = (accountingMatch ? accountingMatch[1] : standardMatch[2]).replace(/,/g, '');
  const sign = accountingMatch || standardMatch[1] === '-' ? -1 : 1;
  const number = sign * Number(numericText);
...
function createSwingLeadersService({
  ...
  async function refresh() {
    ...
    try {
      ...
      snapshot.equity_history_stale = equityHistoryStale;
      snapshot.equity_history_latest_date = lastValidEquityHistory?.length
        ? lastValidEquityHistory[lastValidEquityHistory.length - 1].snapshot_date
        : null;
...
function renderModelPnlChart(items, { stale = false } = {}) {
  ...
  const staleNotice = stale
    ? '<p class="equity-chart-stale">Equity History temporarily stale — showing last valid snapshot</p>'
    : '';
...
<body>
  ...
  <main class="wrap">
    <div class="hero-layout">
      ...
      ${renderModelPnlChart(data.equity_history, { stale: data.equity_history_stale === true })}
