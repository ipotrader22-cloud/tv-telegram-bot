...
function verificationSentHtml({ email = "", name = "", lang = "en" } = {}) {
  ...
  const text = isRu
    ? `Мы отправили одноразовую ссылку подтверждения на <strong>${safeEmail}</strong>. Проверьте Входящие и папку Спам/Нежелательная почта. После подтверждения заявка попадёт на ручную проверку. Доступ не предоставляется автоматически.`
    : `We sent a one-time confirmation link to <strong>${safeEmail}</strong>. Check your Inbox and Spam/Junk folders if you do not see it. After you confirm, your request will enter manual review. Access is not granted automatically.`;
...
function verifiedHtml({ lang = "en" } = {}) {
  const isRu = String(lang).toLowerCase() === "ru";
  const title = isRu ? "Email подтверждён." : "Your email is confirmed.";
  const text = isRu
    ? "Ваша заявка на доступ ожидает ручной проверки. Доступ не предоставляется автоматически. Если заявка будет одобрена, вы получите отдельное письмо с персональным viewer code."
    : "Your access request is awaiting manual review. Access is not granted automatically. If approved, you will receive a separate email with your personal viewer code.";
