...
function verificationSentHtml({ email = "", name = "", lang = "en" } = {}) {
  ...
  const inboxNote = isRu
    ? "Проверьте папки «Входящие» и «Спам»/«Нежелательная почта», если письмо не появилось сразу."
    : "Check your Inbox and Spam/Junk folder if the confirmation email does not appear right away.";
  return pageHtml({ lang: isRu ? "ru" : "en", title, heading: title, paragraphs: [greeting, text, inboxNote].filter(Boolean), href: "/", button: isRu ? "На Vixale" : "Back to Vixale" });
}
...
function verifiedHtml({ lang = "en" } = {}) {
  ...
  const text = isRu
    ? "Ваш email подтверждён. Заявка на доступ ожидает ручной проверки. Доступ не предоставляется автоматически. Если заявка будет одобрена, персональный viewer code придёт по email."
    : "Your email is confirmed. Your access request is awaiting manual review. Access is not granted automatically. If approved, you will receive your personal viewer code by email.";
