...
path = Path("scripts/qa-production-locales.js")
text = path.read_text()
old_route = '''  {
...
if old_route not in text:
    raise SystemExit("Options route QA anchor not found")
text = text.replace(old_route, new_route, 1)

screenshot_anchor = '''          await Promise.all([
            enPage.screenshot({ path: path.join(screenshotsDir, `${viewportName}-${slug}-en.png`), fullPage: true }),'''
options_block = r'''          if (route.path === "/trading-systems/options") {
...
'''
if screenshot_anchor not in text:
    raise SystemExit("Options screenshot QA insertion anchor not found")
text = text.replace(screenshot_anchor, options_block + screenshot_anchor, 1)
