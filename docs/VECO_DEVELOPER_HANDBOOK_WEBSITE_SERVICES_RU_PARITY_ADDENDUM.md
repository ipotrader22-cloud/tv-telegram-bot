...
The post-merge PR #179 production run exposed a separate stale-QA assumption in the Automation / Setup `<select>`. The current renderer starts with the actionable `automation` option and no longer emits the historical disabled `Select a topic…` placeholder option. EN and RU production screenshots both show the automation choice as the initial visible selection, and DOM QA did not find a corresponding placeholder option. Therefore the current production contract requires the three live option values/labels (`automation`, `setup`, `other`) and does not require a nonexistent placeholder option. The legacy `Select a topic…` translation mapping remains covered separately so older markup would still localize safely if encountered.
