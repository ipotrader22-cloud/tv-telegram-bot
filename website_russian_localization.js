...
function localizeSeoHosts(html, pathname) {
  ...
  // Remove only alternates generated by this layer before rebuilding them. Keep
  // whitespace that existed before the generated block so repeated localization
  // is byte-idempotent rather than accumulating/removing line breaks.
  out = out.replace(/<link\b[^>]*data-vx-ru-hreflang=["'](?:1|ru|x-default)["'][^>]*>\n?/gi, "");
