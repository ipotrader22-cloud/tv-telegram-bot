...
assert(hero.includes("After successful registration dashboard access will be emailed within 24 hours"));
assert(!hero.includes("Read-only viewer access · Email verification · Manual review"));
...
const coverageShell='<!doctype html><html><head></head><body><section class="vx-home-day-trading"><div class="wrap"><div class="vx-home-equity-head"><div><p>Day Trading closed trades only · Open P&amp;L excluded</p></div></div></div></section></body></html>';
const coverage=transformHomepagePerformanceTruth(coverageShell,"/");
assert(coverage.includes("coverage.textContent=range+'. '+included+' closed trade'+(included===1?'':'s')"));
assert(!coverage.includes("omitted_row_count"));
