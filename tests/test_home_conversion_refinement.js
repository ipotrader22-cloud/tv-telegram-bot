...
assert(!out.includes("After successful registration dashboard access will be emailed within 24 hours"));
assert(out.includes("Read-only viewer access · Email verification · Manual review"));
assert(out.includes("read-only viewer access or a separate Vixale service is useful for you"));
const loginIndex = out.indexOf('class="vx-home-hero-btn primary access" href="/dashboard">Login</a>');
const requestIndex = out.indexOf('class="vx-home-hero-btn primary access" href="/#password-access">Request Free Access</a>');
const resultsIndex = out.indexOf('href="/results">Explore Results</a>');
assert.strictEqual(loginIndex, -1, "hero must not render a full-size Login CTA");
assert(requestIndex >= 0 && requestIndex < resultsIndex, "hero CTA order must be Request Free Access, then Explore Results");
...
assert(accessIndex < riskIndex, "access form must remain before the risk/legal footer");
