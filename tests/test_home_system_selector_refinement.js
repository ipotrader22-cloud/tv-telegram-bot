...
assert(out.includes("How It Works"));
assert(out.includes("Understand Vixale before you request anything."));
assert(out.includes("Explore systems"));
assert(out.includes("Review available evidence"));
assert(out.includes("Request viewer access if useful"));
assert(out.includes("Viewer access is read-only."));
...
assert(out.includes("Learn about Day Trading →"));
assert(out.includes("Learn about Swing Trading →"));
assert(out.includes("Learn about Options →"));
assert(!out.includes("Vixale Prime"));
assert(!out.includes("Vixale Edge"));
assert(!out.includes("Straddles"));
assert(!out.includes("Swing Leaders"), "homepage comparison must stay category-first before internal system names");
...
assert(out.includes("including its freshness state"));
...
const heroIndex = out.indexOf('class="vx-home-hero"');
const howIndex = out.indexOf('class="vx-home-how"');
const compareIndex = out.indexOf('class="vx-home-compare"');
const selectorIndex = out.indexOf('class="vx-home-system-stack"');
const previewIndex = out.indexOf(PREVIEW_MARKER);
const dayIndex = out.indexOf(`id="${DAY_ANCHOR_ID}"`);
assert(heroIndex < howIndex, "How It Works must immediately follow the hero within the top composition");
assert(howIndex < compareIndex, "system comparison must follow How It Works");
assert(compareIndex < selectorIndex, "comparison heading must lead the category cards");
assert(selectorIndex < previewIndex, "authentic evidence preview must follow the beginner comparison");
assert(previewIndex < dayIndex, "preview must precede the detailed Day Trading evidence block");
...
console.log("Homepage first-time visitor hierarchy + authentic preview: PASS");
