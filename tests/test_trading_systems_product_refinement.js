...
const {SYSTEMS_PATH,DAY_PATH,SWING_PATH,OPTIONS_PATH,OPTIONS_VIEWER_PATH,RESULTS_PATH,STYLE_ID,PAGE_MARKER,refineTradingSystemsProductPage}=require("../website_trading_systems_product_refinement");
...
assert(hub.includes(`href="${RESULTS_PATH}">Open Results</a>`));
...
assert(hub.includes("Public overview; viewer access required for journal details"));
...
assert(options.includes(`<a class="vx-systems-btn primary" href="${OPTIONS_VIEWER_PATH}">Options Viewer</a>`));
assert(!options.includes("Already have access? Open Options"));
...
assert(options.includes(".vx-systems-btn.primary{border-color:#078f51;background:#078f51;color:#fff}"), "Options access action should reuse the canonical filled green CTA class");
assert(options.includes(".vx-systems-actions,.vx-performance-links{flex-direction:column}.vx-systems-btn,.vx-mini-link{width:100%"), "Options hero actions should remain mobile-safe");
